package main

import (
	"context"
	"crypto/ecdsa"
	"fmt"
	"log"
	"github.com/astra-x/go-ethereum/common"
	"github.com/astra-x/go-ethereum/crypto"
	"github.com/astra-x/go-ethereum/ethclient"
	//"time"
	"math/big"
	"github.com/astra-x/go-ethereum/core/types"
	"time"
)

const (
	HTTP = "http://127.0.0.1:8545"
	address_to = "0xd08a05283ad35600ab448c08db31a7c3797c8319"
	priv_key = "d81952d9449a63525e2ef643e1b4ef7be924ac5a37602f00677c9940fa20d4cf"
	tps = 1000
	test_size = 30000
	)



func sendTransactions(client *ethclient.Client, addr_to string, priv_k string)  {
	//signedTxs := make(chan *types.Transaction,1)
	privateKey, err := crypto.HexToECDSA(priv_k)
	if err != nil {
		log.Fatal(err)
	}
	publicKey := privateKey.Public()
	publicKeyECDSA, ok := publicKey.(*ecdsa.PublicKey)
	if !ok {
		log.Fatal("error casting public key to ECDSA")
	}
	fromAddress := crypto.PubkeyToAddress(*publicKeyECDSA)
	fmt.Println(common.Bytes2Hex(fromAddress[:]))
	if err != nil {
		log.Fatal(err)
	}
	value := big.NewInt(10000)      // in wei
	gasLimit := big.NewInt(210000) // in units
	gasPrice, err := client.SuggestGasPrice(context.Background())

	if err != nil {
		log.Fatal(err)
	}
	toAddress := common.HexToAddress(addr_to)
	var data []byte
	nonce, err := client.NonceAt(context.Background(), fromAddress, nil)
	balance, _:= client.BalanceAt(context.Background(), fromAddress, nil)
	fmt.Println("Balance of the sender:  ", balance)
	// Add 2048 digit HEX of random data for heavy io
	//data = common.Hex2Bytes("")

	//Start iterating
	//speed := time.Duration(time.Second/tps)
	var tx_receipts []common.Hash
	ticker:= time.NewTicker((1000/tps)*time.Millisecond)
	for i:=0; i < test_size;i++ {

		tx := types.NewTransaction(0, nonce, toAddress, value, gasLimit, gasPrice, data)
		chainID, err := client.NetworkID(context.Background())
		if err != nil {
			log.Fatal(err)
		}

		signedTx, err := types.SignTx(tx, types.NewEIP155Signer(chainID), privateKey)
		//fmt.Println("signed tx", signedTx)
		if err != nil {
			log.Fatal(err)
		}

		err = client.SendTransaction(context.Background(), signedTx)
		if err != nil {
			log.Fatal(err)
		}
		fmt.Println("tx hash: ", signedTx.Hash().Hex())
		tx_receipts= append(tx_receipts, signedTx.Hash())

		nonce = nonce + 1
		select {
			case <-ticker.C:
		}
		//fmt.Println("Balance  :", balance)
		//fmt.Println(nonce)
		}
	for _, hash :=range tx_receipts {
		reciept, err:=client.TransactionReceipt(context.Background(), hash)
		if err != nil {
			log.Fatal(err)
		}
		fmt.Println("Transaction receipt: ", reciept.TxHash.Hex())
	}
	}

//func getTransactionCount(client *ethclient.Client) uint {
//	ticker:= time.NewTicker(time.Second)
//	for {
//		blockhash, err:= client.BlockByNumber(context.Background(), nil)
//		if err != nil {
//			log.Fatal(err)
//		}
//		proc_txs, err:= client.TransactionCount(context.Background(),blockhash.Hash())
//		if err != nil {
//			log.Fatal(err)
//		}
//		fmt.Println("hello")
//
//		select {
//		case <-ticker.C:
//		}
//		return proc_txs
//	}
//
//}

//func runParallel(client *ethclient.Client) (uint,uint64) {
//	tx_count := make(chan uint, 1)
//	nonce := make(chan uint64, 1)
//	go func() {
//		tx_count <- getTransactionCount(client)
//	}()
//	go func() {
//		nonce <- sendTransactions(client)
//	}()
//	return <- tx_count, <-nonce
//}

func main() {
	client, err:= ethclient.Dial(HTTP)
	if err != nil {
		log.Fatal(err)
	}
	sendTransactions(client, address_to, priv_key)


}
